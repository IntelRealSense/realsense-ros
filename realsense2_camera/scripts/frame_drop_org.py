...
logging.basicConfig(filename=test_log_path, level=logging.INFO,
...
def save_test_results(current_iteration, test_results_data):
    with open(test_results_data_path, "a") as f:
        if current_iteration == 0:
            f.write(
...
def gyro_metadata_cb(msg):
    metadata_cb(msg, StreamType.GYRO)
...
def service_call(node, client, service, service_msg, request):
    if not node:
...
def set_parameters(node, client, request: SetParameters.Request):
    result = service_call(node, client, '/camera/set_parameters', SetParameters, request)
...
def disable_stream(node, client, stream_type):
    ...
    req.parameters.append(Parameter(name=query,
                                    value=ParameterValue(type=ParameterType.PARAMETER_BOOL,
                                                         bool_value=val)))
    set_parameters(node, client, req)


def enable_stream(node, client, stream_type):
    req = SetParameters.Request()
...
def main():
    ...
    accel_sub = node.create_subscription(Metadata, '/accel/metadata', accel_metadata_cb,
                                         custom_qos)
    color_sub = node.create_subscription(Metadata, '/color/metadata', color_metadata_cb,
                                         custom_qos)
    depth_sub = node.create_subscription(Metadata, '/depth/metadata', depth_metadata_cb,
                                         custom_qos)
    gyro_sub = node.create_subscription(Metadata, '/gyro/metadata', gyro_metadata_cb, custom_qos)
    ir1_sub = node.create_subscription(Metadata, '/infra1/metadata', infra1_metadata_cb,
                                       custom_qos)
    ir2_sub = node.create_subscription(Metadata, '/infra2/metadata', infra2_metadata_cb,
                                       custom_qos)
    ...
    for i in range(iterations):
        ...
        frame_collect = True
        logging.debug(f'Starting {StreamType.COLOR.value} stream')
        enable_stream(node, client, StreamType.COLOR.value)
        logging.debug(f'Starting {StreamType.DEPTH.value} stream')
        enable_stream(node, client, StreamType.DEPTH.value)
        logging.debug(f'Starting {StreamType.IR1.value} stream')
        enable_stream(node, client, StreamType.IR1.value)
        logging.debug(f'Starting {StreamType.ACCEL.value} stream')
        enable_stream(node, client, StreamType.ACCEL.value)
        logging.debug(f'Starting {StreamType.GYRO.value} stream')
        enable_stream(node, client, StreamType.GYRO.value)

        
        logging.info(f'collecting frames for {stream_duration} seconds')
        
        time.sleep(stream_duration)
        
        frame_collect = False
        
        logging.debug(f'Stopping {StreamType.COLOR.value} stream')
        disable_stream(node, client, StreamType.COLOR.value)
        logging.debug(f'Stopping {StreamType.DEPTH.value} stream')
        disable_stream(node, client, StreamType.DEPTH.value)
        logging.debug(f'Stopping {StreamType.IR1.value} stream')
        disable_stream(node, client, StreamType.IR1.value)
        logging.debug(f'Stopping {StreamType.ACCEL.value} stream')
        disable_stream(node, client, StreamType.ACCEL.value)
        logging.debug(f'Stopping {StreamType.GYRO.value} stream')
        disable_stream(node, client, StreamType.GYRO.value)
        
        time.sleep(3)
        
        # Save frames raw data
        #logging.info('Saving frames raw data')
        #save_raw_data(i, raw_data_frames)
        ...
        save_test_results(i, test_result)

        raw_data_frames[StreamType.COLOR].clear()
        raw_data_frames[StreamType.DEPTH].clear()
        raw_data_frames[StreamType.IR1].clear()
        raw_data_frames[StreamType.IR2].clear()
        raw_data_frames[StreamType.ACCEL].clear()
        raw_data_frames[StreamType.GYRO].clear()
