      ...
      _logger(this->get_logger())
{
    ...
    ROS_INFO_STREAM("Intra-Process is " << (this->get_node_options().use_intra_process_comms() ? "ON" : "OFF"));
    // Create a subscription on the input topic.
    sub_ = this->create_subscription<sensor_msgs::msg::Image>(
        "/color/image_raw", // TODO Currently color only, we can declare and accept the required streams as ros parameters
        ...
        [&, this](const sensor_msgs::msg::Image::SharedPtr msg)
        {
            ...
            ROS_INFO_STREAM("Got msg with address 0x" << std::hex << reinterpret_cast<std::uintptr_t>(msg.get()) << std::dec << " with latency of " << latency  << " [sec]");
        });
}
