...
def usage():
    ...
    print( '                        ex: --device=d455,d435i,d585 (or) --device d455,d435i,d585 ')
...
def run_test(cmd, test=None, dev_name=None, stdout=None, append =False):
        ...
        try:
            if test:
                stdout = stdout + os.sep + str(dev_name) + '_' + test + '.log'
            else:
                stdout = stdout + os.sep + str(dev_name) + '_' + 'full.log' 
            if stdout is None:
                sys.stdout.flush()
            ...
            if handle:
                handle.close()      
            junit_xml_parsing(f'{dev_name}_pytest.xml')
...
def junit_xml_parsing(xml_file):
    ...
    if not os.path.isfile( os.path.join(logdir, f'{xml_file}' )):
        log.e("{xml_file} not found, test resutls can't be generated")
    else:
        ...
        if device_set:
            for device in device_set:
                if device.upper() in connected_devices:
                   ...
                   log.i('Skipping run on device:', device, ', -- NOT detected' ) 
        else:
            #Loop in for all connected devices and run tests
...
if __name__ == '__main__':
    try:
        opts, args = getopt.getopt( sys.argv[1:], 'hr:', longopts=['help', 'debug', 'regex=', 'device=' ] )
